...
- 📚 **多彩なカテゴリ**: セキュリティ、IT基礎、プログラミング、プロジェクトマネージメント（合計83問）
...
v1.1以降、問題データはPythonコードから分離され、JSONファイルで管理されています。
...
├── questions/                   # 問題データ
│   ├── security.json            # セキュリティ（23問）
│   ├── it_basics.json           # IT基礎（20問）
│   ├── programming.json         # プログラミング（20問）
│   └── project_management.json  # プロジェクトマネージメント（20問）
└── terms/                       # 用語参考
    ├── security.json            # セキュリティ用語（5件）
    ├── it_basics.json           # IT基礎用語（5件）
    ├── programming.json         # プログラミング用語（5件）
    └── project_management.json  # プロジェクトマネージメント用語（5件）
```

### 問題の追加・編集

1. 該当カテゴリのJSONファイルを編集（`data/questions/*.json`）
2. 更新スクリプトを実行：
